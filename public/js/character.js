window.TubloxCharacter = (function() {
  ...
  function draw(c, x, y, w, h, dir, state, frame, avatar, username, isMe, time, options) {
    ...
    // Accessory behind (cape, wings, backpack)
  ...
  function drawBody(c, ox, oy, headW, headH, torsoW, torsoH, legW, legH, armW, armH, totalH, bc, hc, ec, state, frame, bobY, time, attacking, attackProgress, activeItem, isDead, itemOn, equipped) {
    ...
    if (state === 'run') {
      ...
      drawLeg(c, legW, legH, pantsType, pantsCol); c.restore();
      c.save(); c.translate(legW/2+1, legTop-bobY); c.rotate(-ls*Math.PI/180);
      drawLeg(c, legW, legH, pantsType, pantsCol); c.restore();
    } else if (state === 'jump') {
      c.save(); c.translate(-legW-1, legTop-bobY);
      drawLeg(c, legW, legH-3, pantsType, pantsCol); c.restore();
      c.save(); c.translate(1, legTop-bobY);
      drawLeg(c, legW, legH-3, pantsType, pantsCol); c.restore();
    } else {
      c.save(); c.translate(-legW-1, legTop-bobY);
      drawLeg(c, legW, legH, pantsType, pantsCol); c.restore();
    ...
    // Scarf/Necklace (front accessories)
    if (equipped.accessory) {
    ...
    if (equipped.hair) {
      drawHairCosmetic(c, headW, headH, headTop, bobY, equipped.hair, time);
    }

    // === FACE ===
    const faceEquip = equipped.face;
    const faceType = faceEquip?.drawData?.type;
    ...
    // === HAT ===
    if (equipped.hat) {
      drawHatCosmetic(c, headW, headH, headTop, bobY, equipped.hat, time);
    }
  ...
  function drawTorso(c, tw, th, torsoTop, bobY, shirtType, shirtCol, bc) {
    ...
    switch(shirtType) {
      case 'hoodie':
        c.fillStyle = shade(shirtCol, -20);
        c.fillRect(-tw/2, torsoTop-bobY, tw, 5);
        c.fillStyle = shade(shirtCol, -10);
        c.fillRect(-4, torsoTop-bobY+th*0.3, 8, 10);
        break;
      case 'jacket':
        c.fillStyle = shade(shirtCol, -15);
        c.fillRect(-1, torsoTop-bobY, 2, th);
        c.fillStyle = shade(shirtCol, 15);
        c.fillRect(-tw/2+1, torsoTop-bobY+th*0.3, 5, 8);
        break;
      case 'stripe_shirt':
        c.fillStyle = 'rgba(255,255,255,0.15)';
        for (let y = 0; y < th; y += 5) c.fillRect(-tw/2, torsoTop-bobY+y, tw, 2);
        break;
      case 'gold_armor':
        c.fillStyle = '#DAA520';
        c.fillRect(-tw/2, torsoTop-bobY, tw, 3);
        c.fillRect(-tw/2, torsoTop-bobY+th-3, tw, 3);
        c.fillStyle = 'rgba(255,255,255,0.1)';
        c.fillRect(-tw/2, torsoTop-bobY, 4, th);
        break;
      case 'tank_top':
        c.fillStyle = shade(shirtCol, -10);
        c.fillRect(-tw/2, torsoTop-bobY, tw, 2);
        break;
      default: // basic_tee
        c.fillStyle = shade(shirtCol, 10);
        c.fillRect(-tw/2, torsoTop-bobY, tw, 2);
  ...
  function drawFaceCosmetic(c, headW, headH, headTop, bobY, type, ec, time, isDead) {
    const cy = headTop + headH*0.35 - bobY;
    const eyeSize = Math.max(2, Math.round(headW*0.14));
    const gap = Math.round(headW*0.12);
    const blink = Math.sin(time*2.5) > 0.93;

    switch(type) {
      case 'smile':
        if (!blink) { c.fillStyle = '#000'; c.beginPath(); c.arc(-gap, cy, eyeSize, 0, Math.PI*2); c.fill(); c.beginPath(); c.arc(gap, cy, eyeSize, 0, Math.PI*2); c.fill(); }
        else { c.fillStyle = '#000'; c.fillRect(-gap-eyeSize, cy, eyeSize*2, 2); c.fillRect(gap-eyeSize, cy, eyeSize*2, 2); }
        c.strokeStyle = '#000'; c.lineWidth = 1.5;
        c.beginPath(); c.arc(0, cy+6, headW*0.2, 0.2, Math.PI-0.2); c.stroke();
        break;
      case 'cool':
        c.fillStyle = '#111'; c.fillRect(-headW*0.4, cy-2, headW*0.8, eyeSize+3);
        c.fillStyle = '#333'; c.fillRect(-headW*0.35, cy, headW*0.28, eyeSize); c.fillRect(headW*0.07, cy, headW*0.28, eyeSize);
        c.strokeStyle = '#000'; c.lineWidth = 1; c.beginPath(); c.arc(0, cy+8, headW*0.15, 0.3, Math.PI-0.3); c.stroke();
        break;
      case 'angry':
        if (!blink) { c.fillStyle = '#000'; c.beginPath(); c.arc(-gap, cy, eyeSize, 0, Math.PI*2); c.fill(); c.beginPath(); c.arc(gap, cy, eyeSize, 0, Math.PI*2); c.fill(); }
        c.strokeStyle = '#000'; c.lineWidth = 1.5;
        c.beginPath(); c.moveTo(-gap-eyeSize, cy-4); c.lineTo(-gap+eyeSize, cy-1); c.stroke();
        c.beginPath(); c.moveTo(gap+eyeSize, cy-4); c.lineTo(gap-eyeSize, cy-1); c.stroke();
        c.beginPath(); c.arc(0, cy+10, headW*0.12, Math.PI+0.3, -0.3); c.stroke();
        break;
      case 'wink':
        if (!blink) { c.fillStyle = '#000'; c.beginPath(); c.arc(-gap, cy, eyeSize, 0, Math.PI*2); c.fill(); }
        c.strokeStyle = '#000'; c.lineWidth = 1.5;
        c.beginPath(); c.moveTo(gap-eyeSize, cy); c.lineTo(gap+eyeSize, cy); c.stroke();
        c.beginPath(); c.arc(0, cy+6, headW*0.15, 0.2, Math.PI-0.2); c.stroke();
        break;
      case 'robot':
        c.fillStyle = '#00FF00'; c.fillRect(-gap-2, cy-1, 4, 4); c.fillRect(gap-2, cy-1, 4, 4);
        c.fillStyle = '#00CC00'; c.fillRect(-headW*0.25, cy+6, headW*0.5, 2); c.fillRect(-headW*0.18, cy+10, headW*0.36, 2);
        break;
      case 'skull':
        c.fillStyle = '#000'; c.beginPath(); c.arc(-gap, cy, eyeSize+1, 0, Math.PI*2); c.fill(); c.beginPath(); c.arc(gap, cy, eyeSize+1, 0, Math.PI*2); c.fill();
        c.fillRect(-2, cy+4, 4, 5);
        c.fillStyle = '#888';
        for (let i = -2; i <= 2; i++) c.fillRect(i*3-1, cy+12, 2, 4);
        break;
      default:
        // Default eyes
        if (!blink) {
          c.fillStyle = '#111'; c.fillRect(gap, cy, eyeSize+1, eyeSize+2); c.fillRect(-gap-eyeSize-1, cy, eyeSize+1, eyeSize+2);
          c.fillStyle = ec; c.fillRect(gap+1, cy+1, eyeSize, eyeSize); c.fillRect(-gap-eyeSize, cy+1, eyeSize, eyeSize);
        }
        c.fillStyle = 'rgba(0,0,0,0.3)'; c.fillRect(-2, headTop+headH*0.75-bobY, 4, 2);
    }
  }
    ...
    switch(type) {
      ...
      case 'curly':
        for (let i = 0; i < 8; i++) {
          ...
          c.beginPath(); c.arc(Math.cos(angle)*headW*0.35, ht+headH*0.25+Math.sin(angle)*headH*0.3, headW*0.15, 0, Math.PI*2); c.fill();
        }
        break;
      case 'fire':
  ...
  function drawHatCosmetic(c, headW, headH, headTop, bobY, hatEquip, time) {
    ...
    switch(type) {
      ...
      case 'crown':
        ...
        const points = [-0.4,-0.2,0,0.2,0.4];
        points.forEach((p,i) => {
          if (i%2 === 0) {
            c.beginPath();
        ...
        c.fillStyle = '#EF4444'; c.beginPath(); c.arc(0, ht-headH*0.25, 2, 0, Math.PI*2); c.fill();
        break;
      case 'beanie':
  ...
  function drawAccessoryBehind(c, ox, oy, tw, th, lh, hh, bobY, type, col, time, state) {
    ...
    switch(type) {
      case 'cape':
        c.fillStyle = col;
        const capeWave = Math.sin(time*3)*3 + (state === 'run' ? 5 : 0);
        c.beginPath();
        c.moveTo(-tw*0.4, torsoTop-bobY+3);
        c.lineTo(tw*0.4, torsoTop-bobY+3);
        c.quadraticCurveTo(tw*0.3, -bobY+capeWave, tw*0.35, -bobY+lh*0.5);
        c.lineTo(-tw*0.35, -bobY+lh*0.5);
        c.quadraticCurveTo(-tw*0.3, -bobY+capeWave, -tw*0.4, torsoTop-bobY+3);
        c.fill();
        break;
      case 'wings':
        ...
        // Left
        c.save(); c.rotate(-0.3+wingFlap);
        c.beginPath(); c.moveTo(-tw*0.3, torsoTop-bobY+th*0.3);
        c.quadraticCurveTo(-tw*1.2, torsoTop-bobY-hh, -tw*0.8, torsoTop-bobY+th*0.6);
        c.fill(); c.restore();
        // Right
        c.save(); c.rotate(0.3-wingFlap);
        c.beginPath(); c.moveTo(tw*0.3, torsoTop-bobY+th*0.3);
  ...
  function drawTailCosmetic(c, tw, th, lh, bobY, col, time) {
    const tailBaseY = -lh - 5 - bobY;
    const wag = Math.sin(time*3)*0.3;
    c.strokeStyle = col; c.lineWidth = 4; c.lineCap = 'round';
    c.beginPath();
    c.moveTo(-tw*0.4, tailBaseY);
    c.quadraticCurveTo(-tw*0.8+Math.sin(time*3)*5, tailBaseY-15, -tw*0.6+wag*10, tailBaseY-25);
    c.stroke();
    c.fillStyle = shade(col, 20);
    c.beginPath(); c.arc(-tw*0.6+wag*10, tailBaseY-25, 3, 0, Math.PI*2); c.fill();
  }
  ...
  function drawFlashlight(c, cx, sy, len, isOn, time) {
    ...
    } else { c.fillStyle = '#333'; c.fillRect(cx-3, sy+len+2, 6, 2); }
  ...
  function drawSidebar(canvas, avatar, equipped) {
    ...
    if (equipped?.face) {
      ...
      if (ft === 'cool') { c.fillStyle = '#111'; c.fillRect(13, 5, 14, 5); c.fillStyle = '#333'; c.fillRect(14, 6, 5, 3); c.fillRect(21, 6, 5, 3); }
    }
  }
