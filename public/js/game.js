(function() {
  ...
  let noteHolding = false;

  let wAvatars = [];
  let avatarInteractCool = 0;
  ...
  const IV = {
    ...
    speed_boost: { name: 'Speed Boost', color: '#FFD700', tog: false, dW(c, x, y, t) { c.save(); c.translate(x, y + Math.sin(t * 4) * 2); c.fillStyle = '#FFD700'; c.beginPath(); c.moveTo(2, -12); c.lineTo(-4, 0); c.lineTo(0, 0); c.lineTo(-2, 12); c.lineTo(6, 0); c.lineTo(2, 0); c.closePath(); c.fill(); c.restore(); }, dH(c, x, y, s) { c.save(); c.translate(x + s / 2, y + s / 2); const r = s / 52; c.fillStyle = '#FFD700'; c.beginPath(); c.moveTo(2 * r, -10 * r); c.lineTo(-4 * r, 0); c.lineTo(0, 0); c.lineTo(-2 * r, 10 * r); c.lineTo(6 * r, 0); c.lineTo(2 * r, 0); c.closePath(); c.fill(); c.restore(); }, onC(p, it) { fx.speed = { m: it.multiplier || 1.5, end: Date.now() + (it.duration || 5000) }; sysMsg(`Speed x${it.multiplier || 1.5}!`); }, dE(c, p, t) { if (!fx.speed || Date.now() > fx.speed.end) { delete fx.speed; return; } const px = p.x + p.width / 2, py = p.y + p.height; for (let i = 0; i < 3; i++) { const o = (t * 200 + i * 40) % 60; c.fillStyle = `rgba(255,215,0,${(1 - o / 60) * 0.4})`; c.fillRect(px - p.direction * (o + 10), py - 10 - i * 8, 12, 2); } } },
    jump_boost: { name: 'Jump Boost', color: '#44CC44', tog: false, dW(c, x, y, t) { c.save(); c.translate(x, y + Math.sin(t * 3) * 3); c.fillStyle = '#44CC44'; c.fillRect(-6, 4, 12, 4); c.fillStyle = '#66EE66'; c.beginPath(); c.moveTo(0, -14); c.lineTo(5, -8); c.lineTo(-5, -8); c.closePath(); c.fill(); c.restore(); }, dH(c, x, y, s) { c.save(); c.translate(x + s / 2, y + s / 2); const r = s / 52; c.fillStyle = '#44CC44'; c.fillRect(-5 * r, 2 * r, 10 * r, 3 * r); c.fillStyle = '#66EE66'; c.beginPath(); c.moveTo(0, -8 * r); c.lineTo(5 * r, -2 * r); c.lineTo(-5 * r, -2 * r); c.closePath(); c.fill(); c.restore(); }, onC(p, it) { fx.jump = { m: it.multiplier || 1.5, end: Date.now() + (it.duration || 5000) }; sysMsg(`Jump x${it.multiplier || 1.5}!`); }, dE(c, p, t) { if (!fx.jump || Date.now() > fx.jump.end) { delete fx.jump; return; } const px = p.x + p.width / 2, py = p.y + p.height; for (let i = 0; i < 4; i++) { const a2 = t * 5 + i * 1.571, r = 8 + Math.sin(t * 8 + i) * 4; c.fillStyle = `rgba(68,204,68,${0.3 + Math.sin(t * 4 + i) * 0.15})`; c.fillRect(px + Math.cos(a2) * r - 1.5, py + Math.sin(t * 6 + i) * 3 - 1.5, 3, 3); } } },
    ...
    note: {
      ...
      onC(p, it) {
        ...
        collectedNotes.push({ text: t, id: 'note_' + Date.now() + '_' + collectedNotes.length });
        currentNoteIndex = collectedNotes.length - 1;
  ...
  function showNote(indexOrText) {
    let idx;
    if (typeof indexOrText === 'number') idx = indexOrText;
    else if (typeof indexOrText === 'string') { idx = collectedNotes.findIndex(n => n.text === indexOrText); if (idx === -1) idx = collectedNotes.length - 1; }
    else idx = collectedNotes.length - 1;
    if (idx < 0 || idx >= collectedNotes.length) return;

    noteReading = true;
    currentNoteIndex = idx;
    noteText = collectedNotes[idx].text;
    ...
    let hintText = '';
    if (collectedNotes.length > 1) {
      hintText = `Note ${idx + 1} / ${collectedNotes.length}  •  `;
      hintText += mobile ? 'Swipe ◀▶ to switch  •  Tap X to close' : '← → switch  •  N or Esc to close';
    } else {
      hintText = mobile ? 'Tap X to close' : 'N or Esc to close';
    }
    hintEl.textContent = hintText;
    overlay.style.display = 'flex';
  }

  function hideNote() { noteReading = false; noteText = ''; noteHolding = false; document.getElementById('note-overlay').style.display = 'none'; }
  function nextNote() { if (!noteReading || collectedNotes.length <= 1) return; currentNoteIndex = (currentNoteIndex + 1) % collectedNotes.length; showNote(currentNoteIndex); }
  function prevNote() { if (!noteReading || collectedNotes.length <= 1) return; currentNoteIndex = (currentNoteIndex - 1 + collectedNotes.length) % collectedNotes.length; showNote(currentNoteIndex); }
  function updateNoteBtn() { if (!mobile) return; const nb = document.getElementById('mobile-btn-note'); if (!nb) return; nb.style.display = collectedNotes.length > 0 ? 'flex' : 'none'; if (collectedNotes.length > 1) nb.textContent = '📜' + collectedNotes.length; else if (collectedNotes.length === 1) nb.textContent = '📜'; }

  // ==================== DIALOGUE SYSTEM ====================

  function startDialogue(av) {
    if (dialogueActive) return;
    if (!av || !av.dialogue) return;
    const d = av.dialogue;
    if (!d.lines || d.lines.length === 0) return;
    const dialogueId = av.id || (av.npcName + '_' + av.x + '_' + av.y);
    if (d.oneTime && dialogueState.completed.has(dialogueId)) return;
    if (d.hasCondition) { const met = checkDialogueCondition(d); if (!met) { if (d.conditionFailText) iMsg(d.conditionFailText); return; } }

    dialogueActive = true;
    dialogueData = { avatar: av, dialogue: d, dialogueId: dialogueId };
    dialogueState.currentLine = 0; dialogueState.typingIndex = 0;
    dialogueState.isTyping = false; dialogueState.showingChoices = false;
    dialogueState.lastDialogueId = dialogueId;
    if (dialogueState.typingTimer) { clearInterval(dialogueState.typingTimer); dialogueState.typingTimer = null; }
  ...
  function checkDialogueCondition(d) {
    if (d.conditionType === 'has_item' && d.conditionParams?.itemType) { if (!me || !me.inventory) return false; return me.inventory.some(it => it && it.id === d.conditionParams.itemType); }
    if (d.conditionType === 'no_item' && d.conditionParams?.itemType) { if (!me || !me.inventory) return true; return !me.inventory.some(it => it && it.id === d.conditionParams.itemType); }
    if (d.conditionType === 'has_note') return collectedNotes.length > 0;
    if (d.conditionType === 'note_count' && d.conditionParams?.count) return collectedNotes.length >= d.conditionParams.count;
    return true;
  }

  function showDialogueLine(idx) {
    if (!dialogueData) return;
    const d = dialogueData.dialogue;
    if (idx < 0) idx = 0;
    if (idx >= d.lines.length) {
      if (d.hasChoices && d.choices && d.choices.length > 0 && !dialogueState.showingChoices) {
        const choiceAfter = d.choiceAfterLine ? (d.choiceAfterLine - 1) : (d.lines.length - 1);
        if (idx > choiceAfter) { showDialogueChoices(); return; }
      }
      endDialogue(); return;
    }
    dialogueState.currentLine = idx;
    ...
    const speakerName = getSpeakerName(line, d);
    const speakerColor = getSpeakerColor(line, d);
    const emotionMap = { neutral: '', happy: '😊', sad: '😢', angry: '😠', surprised: '😮', thinking: '🤔' };
    const emoticon = emotionMap[line.emotion || 'neutral'] || '';
    nameEl.innerHTML = `<span class="d-name-dot" style="background:${speakerColor}"></span><span style="color:${speakerColor}">${esc(speakerName)}</span>${emoticon ? `<span class="d-name-emotion">${emoticon}</span>` : ''}`;
    if (counterEl) counterEl.textContent = `${idx + 1} / ${d.lines.length}`;
    if (continueEl) { continueEl.textContent = mobile ? 'Tap ▸' : 'E / Click ▸'; continueEl.style.display = 'inline'; }
    if (dialogueState.typingTimer) { clearInterval(dialogueState.typingTimer); dialogueState.typingTimer = null; }

    const text = line.text || '...';
    dialogueState.typingIndex = 0; dialogueState.isTyping = true;
    const speed = line.speed || d.typingSpeed || 30;
    const effect = line.effect || 'normal';
    textEl.className = 'dialogue-text'; textEl.innerHTML = '<span class="d-cursor"></span>';

    dialogueState.typingTimer = setInterval(() => {
      if (!dialogueActive || !dialogueData) { clearInterval(dialogueState.typingTimer); dialogueState.typingTimer = null; return; }
      if (dialogueState.typingIndex <= text.length) {
        let displayText = text.substring(0, dialogueState.typingIndex);
        if (effect === 'wave') { displayText = displayText.split('').map((ch, i) => `<span style="animation-delay:${i * 0.05}s">${ch === ' ' ? '&nbsp;' : esc(ch)}</span>`).join(''); textEl.className = 'dialogue-text d-text-wave'; }
        else if (effect === 'shake') textEl.className = 'dialogue-text d-text-shake';
        else if (effect === 'glitch') textEl.className = 'dialogue-text d-text-glitch';
        else textEl.className = 'dialogue-text';
        textEl.innerHTML = (effect === 'wave' ? displayText : esc(displayText)) + '<span class="d-cursor"></span>';
        dialogueState.typingIndex++;
      } else {
        clearInterval(dialogueState.typingTimer); dialogueState.typingTimer = null; dialogueState.isTyping = false;
        if (effect === 'wave') textEl.innerHTML = text.split('').map((ch, i) => `<span style="animation-delay:${i * 0.05}s">${ch === ' ' ? '&nbsp;' : esc(ch)}</span>`).join('');
        else textEl.textContent = text;
        if (d.hasChoices && d.choices && d.choices.length > 0) {
          const choiceAfter = d.choiceAfterLine ? (d.choiceAfterLine - 1) : (d.lines.length - 1);
          if (idx === choiceAfter) setTimeout(() => { if (dialogueActive) showDialogueChoices(); }, 200);
        }
      }
    }, speed);
  }
  ...
  function showDialogueChoices() {
    ...
    if (continueEl) continueEl.textContent = '';
    if (d.choicePrompt) { const promptEl = document.createElement('div'); promptEl.style.cssText = 'font-size:11px;color:#555;margin-bottom:2px;font-style:italic'; promptEl.textContent = d.choicePrompt; choicesEl.appendChild(promptEl); }
    d.choices.forEach((choice, i) => {
      const btn = document.createElement('button'); btn.className = 'dialogue-choice-btn';
      btn.innerHTML = `<span class="d-choice-key">${i + 1}</span>${esc(choice.text || 'Choice ' + (i + 1))}`;
      btn.addEventListener('click', (e) => { e.stopPropagation(); e.preventDefault(); handleDialogueChoice(choice, i); });
      btn.addEventListener('touchstart', (e) => { e.stopPropagation(); }, { passive: true });
      choicesEl.appendChild(btn);
    });
  }

  function handleDialogueChoice(choice, idx) {
    if (!dialogueData) return;
    const choicesEl = document.getElementById('dialogue-choices');
    if (choicesEl) { choicesEl.style.display = 'none'; choicesEl.innerHTML = ''; }
    dialogueState.showingChoices = false;
    if (choice.action === 'give_item' && choice.itemId && me && me.inventory) {
      const es = me.inventory.findIndex(s => s == null);
      if (es !== -1) { const v = gv(choice.itemId); me.inventory[es] = { id: choice.itemId, name: v.name }; socket.emit('collect-item', { item: me.inventory[es] }); updateMB(); updateMS(); }
    }
    if (choice.action === 'jump' && choice.jumpTo) showDialogueLine(Math.max(0, (choice.jumpTo || 1) - 1));
    else if (choice.action === 'end') endDialogue();
    else showDialogueLine(dialogueState.currentLine + 1);
  }

  function advanceDialogue() {
    if (!dialogueActive || !dialogueData) return;
    if (dialogueState.showingChoices) return;
    if (dialogueState.isTyping) {
      if (dialogueState.typingTimer) { clearInterval(dialogueState.typingTimer); dialogueState.typingTimer = null; }
      dialogueState.isTyping = false;
      const d = dialogueData.dialogue, line = d.lines[dialogueState.currentLine];
      if (!line) { endDialogue(); return; }
      const text = line.text || '...', textEl = document.getElementById('dialogue-text');
      if (!textEl) return;
      const effect = line.effect || 'normal';
      if (effect === 'wave') { textEl.innerHTML = text.split('').map((ch, i) => `<span style="animation-delay:${i * 0.05}s">${ch === ' ' ? '&nbsp;' : esc(ch)}</span>`).join(''); textEl.className = 'dialogue-text d-text-wave'; }
      else if (effect === 'shake') { textEl.textContent = text; textEl.className = 'dialogue-text d-text-shake'; }
      else if (effect === 'glitch') { textEl.textContent = text; textEl.className = 'dialogue-text d-text-glitch'; }
      else { textEl.textContent = text; textEl.className = 'dialogue-text'; }
      if (d.hasChoices && d.choices && d.choices.length > 0) {
        const choiceAfter = d.choiceAfterLine ? (d.choiceAfterLine - 1) : (d.lines.length - 1);
        if (dialogueState.currentLine === choiceAfter) setTimeout(() => { if (dialogueActive) showDialogueChoices(); }, 100);
      }
    } else showDialogueLine(dialogueState.currentLine + 1);
  }
  ...
  function endDialogue() {
    if (dialogueState.typingTimer) { clearInterval(dialogueState.typingTimer); dialogueState.typingTimer = null; }
    if (dialogueData) {
      const d = dialogueData.dialogue, dialogueId = dialogueData.dialogueId;
      if (d.oneTime && dialogueId) dialogueState.completed.add(dialogueId);
      if (d.endAction && d.endAction !== 'none') executeDialogueEndAction(d);
    }
    dialogueActive = false; dialogueData = null;
    dialogueState.currentLine = 0; dialogueState.typingIndex = 0;
    dialogueState.isTyping = false; dialogueState.showingChoices = false; dialogueState.lastDialogueId = null;
    const overlay = document.getElementById('dialogue-overlay'); if (overlay) overlay.style.display = 'none';
    const choicesEl = document.getElementById('dialogue-choices'); if (choicesEl) { choicesEl.style.display = 'none'; choicesEl.innerHTML = ''; }
    const textEl = document.getElementById('dialogue-text'); if (textEl) { textEl.textContent = ''; textEl.className = 'dialogue-text'; }
    const nameEl = document.getElementById('dialogue-name'); if (nameEl) nameEl.innerHTML = '';
  }

  function executeDialogueEndAction(d) {
    if (!d.endActionParams) return;
    if (d.endAction === 'give_item' && d.endActionParams.itemType && me && me.inventory) {
      const es = me.inventory.findIndex(s => s == null);
      if (es !== -1) { const v = gv(d.endActionParams.itemType); me.inventory[es] = { id: d.endActionParams.itemType, name: v.name }; socket.emit('collect-item', { item: me.inventory[es] }); updateMB(); updateMS(); }
    } else if (d.endAction === 'open_door' && d.endActionParams.doorId) openD(d.endActionParams.doorId);
    else if (d.endAction === 'teleport' && d.endActionParams.x !== undefined && me) { me.x = d.endActionParams.x || 0; me.y = d.endActionParams.y || 0; me.vx = 0; me.vy = 0; }
    else if (d.endAction === 'play_anim' && d.endActionParams.anim) { const av = dialogueData?.avatar; if (av) av.defaultAnim = d.endActionParams.anim; }
  }

  function getSpeakerName(line, d) { const s = line.speaker || 'npc'; if (s === 'npc') return d.npcName || dialogueData?.avatar?.npcName || 'NPC'; if (s === 'player') return me?.username || 'You'; if (s === 'narrator') return 'Narrator'; if (s === 'system') return 'System'; return s; }
  function getSpeakerColor(line, d) { const s = line.speaker || 'npc'; if (s === 'npc') return d.nameColor || '#fff'; if (s === 'player') return '#aaa'; if (s === 'narrator') return '#666'; if (s === 'system') return '#888'; return '#888'; }

  const dialogueOverlay = document.getElementById('dialogue-overlay');
  if (dialogueOverlay) {
    dialogueOverlay.addEventListener('click', (e) => { if (e.target.closest('.dialogue-choice-btn')) return; if (dialogueState.showingChoices) return; advanceDialogue(); });
    dialogueOverlay.addEventListener('touchend', (e) => { if (e.target.closest('.dialogue-choice-btn')) return; if (dialogueState.showingChoices) return; e.preventDefault(); advanceDialogue(); }, { passive: false });
  }
  ...
  function applyEasing(t, type) {
    switch (type) {
      ...
      case 'bounce': if (t < 1/2.75) return 7.5625*t*t; if (t < 2/2.75) return 7.5625*(t-=1.5/2.75)*t+0.75; if (t < 2.5/2.75) return 7.5625*(t-=2.25/2.75)*t+0.9375; return 7.5625*(t-=2.625/2.75)*t+0.984375;
      default: return t;
    }
  }

  function drawGameAvatar(c, av, time) {
    const w = av.w || 22, h = av.h || 34;
    let dir = av.direction || 1;
    const animType = av.defaultAnim || 'idle', speed = av.animSpeed || 1;
    let drawX = av.x, drawY = av.y, pose = animType, animOff = 0;

    if (animType === 'custom' && av.keyframes && av.keyframes.length > 0) {
      const total = av.keyframes.reduce((m, kf) => Math.max(m, kf.time + (kf.duration || 0.5)), 1);
      const lt = av.loop !== false ? (time * speed) % total : Math.min(time * speed, total);
      let akf = av.keyframes[0];
      for (let i = av.keyframes.length - 1; i >= 0; i--) { if (lt >= av.keyframes[i].time) { akf = av.keyframes[i]; break; } }
      if (akf) { pose = akf.pose || 'idle'; const prog = Math.min(1, (lt - akf.time) / (akf.duration || 0.5)); const eased = applyEasing(prog, akf.easing || 'linear'); drawX = av.x + (akf.dx || 0) * eased; drawY = av.y + (akf.dy || 0) * eased; if (akf.dir && akf.dir !== 0) dir = akf.dir; }
    }
    ...
    c.save(); c.translate(drawX, drawY + animOff);
    if (av.usePlayerAvatar && me && TC) { TC.draw(c, -w/2, -h/2, w, h, dir, pose === 'run' ? 'run' : pose === 'jump' ? 'jump' : 'idle', pose === 'run' ? Math.floor(time * speed * 8) % 4 : 0, me.avatar, null, false, time, { equipped: me.equipped || {} }); c.restore(); return; }

    const bc = av.bodyColor || '#ffffff', hc = av.headColor || '#ffffff', ec = av.eyeColor || '#000000';
    const bw = w * 0.55, bh = h * 0.45, hw2 = w * 0.5, hh2 = w * 0.5;
    c.fillStyle = 'rgba(0,0,0,0.15)'; c.beginPath(); c.ellipse(0, h / 2, w * 0.35, 2, 0, 0, Math.PI * 2); c.fill();
    const legW = bw * 0.35, legH = h * 0.25;
    c.fillStyle = bc;
    if (pose === 'sit') c.fillRect(-bw / 2, h * 0.2, bw, legH * 0.6);
    else { let ll = -legW - 1, rl = 1; if (pose === 'run') { ll += Math.sin(time * speed * 8) * 3; rl -= Math.sin(time * speed * 8) * 3; } c.fillRect(ll, h * 0.25, legW, legH); c.fillRect(rl, h * 0.25, legW, legH); }
    c.fillStyle = bc; c.fillRect(-bw / 2, -h * 0.15, bw, bh);
    c.fillStyle = hc; c.fillRect(-hw2 / 2, -h * 0.5, hw2, hh2);
    const eyeY = -h * 0.35, eyeSz = Math.max(1.5, w * 0.08);
    c.fillStyle = ec;
    ...
    const armW = 3, armH = bh * 0.7; let la = 0, ra = 0;
    ...
    const npcName = av.dialogue?.npcName || av.npcName || '';
    if (!av.usePlayerAvatar && npcName) { c.fillStyle = av.dialogue?.nameColor || '#fff'; c.font = '10px Inter'; c.textAlign = 'center'; c.fillText(npcName, 0, -h / 2 - 8); }
    if (av.interactive && !dialogueActive) { const trigKey = av.dialogue?.triggerKey || 'E'; const pulseA = 0.4 + Math.sin(time * 3) * 0.2; c.fillStyle = `rgba(255,255,255,${pulseA * 0.3})`; c.beginPath(); c.arc(0, -h / 2 - 20, 8, 0, Math.PI * 2); c.fill(); c.fillStyle = `rgba(255,255,255,${pulseA + 0.2})`; c.font = 'bold 9px Inter'; c.textAlign = 'center'; c.textBaseline = 'middle'; c.fillText(trigKey, 0, -h / 2 - 20); }
    c.restore();
  }

  function findNearInteractiveAvatar() {
    if (!me || !wAvatars.length) return null;
    const px = me.x + me.width / 2, py = me.y + me.height / 2;
    let best = null, bd = Infinity;
    for (let i = 0; i < wAvatars.length; i++) { const av = wAvatars[i]; if (!av.interactive) continue; const trigR = av.dialogue?.triggerRadius || 80; const d = Math.sqrt((px - av.x) ** 2 + (py - av.y) ** 2); if (d < trigR && d < bd) { best = av; bd = d; } }
    return best;
  }
  ...
  function drawDoorPanel(c, x, y, w, h, color, dt, props, t) {
    ...
    if (dt === 'key') {
      ...
      const hx = x + w - 10, hy = y + h * 0.5;
      c.fillStyle = '#999'; c.beginPath(); c.arc(hx, hy, 3.5, 0, 6.283); c.fill();
      c.fillStyle = '#bbb'; c.fillRect(hx - 6, hy - 1, 6, 2);
    } else if (dt === 'keycard') {
      const cc = kcCol(props); c.fillStyle = cc; c.fillRect(x, y, 3, h); c.fillRect(x + w - 3, y, 3, h);
  ...
  function drawDoor(c, m, t) {
    ...
    const color = m.properties?.color || (m.type === 'door_key' ? '#8B4513' : m.type === 'door_simple' ? '#666' : '#555');
    let dt = 'key';
    if (m.type === 'door_simple') dt = 'simple';
    else if (m.type === 'door_keycard') dt = 'keycard';
    else if (m.type === 'door_lever') dt = 'lever';
    drawDoorFrame(c, m.x, m.y, mw, mh);
    if (p > 0.01) { c.fillStyle = `rgba(0,0,0,${0.4 + p * 0.4})`; c.fillRect(m.x, m.y, mw, mh); }
    if (p < 0.98) {
      c.save(); c.beginPath(); c.rect(m.x - 5, m.y - 5, mw + 10, mh + 10); c.clip();
      let px = m.x, py = m.y;
      if (dir === 'right') px += mw * p; else if (dir === 'left') px -= mw * p;
      else if (dir === 'up') py -= mh * p; else if (dir === 'down') py += mh * p;
      drawDoorPanel(c, px, py, mw, mh, color, dt, m.properties, t);
      c.restore();
    }
    ...
    if (p < 0.05 && dt !== 'simple') {
      ...
      c.fillText(dt === 'key' ? '🔒' : dt === 'keycard' ? '💳' : '⚙', m.x + mw / 2, m.y - 12);
    }
  }

  function drawLever(c, x, y, w, h, act, t) { if (!inView(x, y, w, h)) return; c.fillStyle = '#444'; c.fillRect(x, y + h - 8, w, 8); c.fillStyle = '#666'; c.beginPath(); c.arc(x + w / 2, y + h - 8, 5, 0, 6.283); c.fill(); const ang = act ? 0.7 : -0.7, lx = x + w / 2 + Math.sin(ang) * (h - 12), ly = y + h - 8 - Math.cos(ang) * (h - 12); c.strokeStyle = '#888'; c.lineWidth = 4; c.beginPath(); c.moveTo(x + w / 2, y + h - 8); c.lineTo(lx, ly); c.stroke(); c.fillStyle = act ? '#44CC44' : '#CC3333'; c.beginPath(); c.arc(lx, ly, 5, 0, 6.283); c.fill(); }
  ...
  function getModelBlocks() {
    ...
    for (let i = 0; i < wModels.length; i++) {
      ...
      const a = doorAnim[m.id], p = a ? a.p : 0;
      if (p >= 0.95) continue;
      const mw = m.w || 40, mh = m.h || 80, dir = m.properties?.direction || 'right';
  ...
  function nearI() {
    ...
    for (let i = 0; i < wModels.length; i++) {
      ...
      const can = m.type === 'lever'
  || m.type === 'door_simple'
  || (m.type === 'door_key' && !doors[m.id])
  || (m.type === 'door_keycard' && !doors[m.id]);
      if (can && d < 60 + Math.max(mw, mh) / 2 && d < bd) { best = m; bd = d; }
    }
    const nearAv = findNearInteractiveAvatar();
    if (nearAv) { const d = Math.sqrt((px - nearAv.x) ** 2 + (py - nearAv.y) ** 2); if (d < bd) { best = nearAv; bd = d; } }
    return best;
  }
  ...
  function interact() {
    ...
    const nearAv = findNearInteractiveAvatar();
    if (nearAv) {
      const d = Math.sqrt((px - nearAv.x) ** 2 + (py - nearAv.y) ** 2);
      if (d < (nearAv.dialogue?.triggerRadius || 80)) {
        if (nearAv.dialogue && nearAv.dialogue.lines && nearAv.dialogue.lines.length > 0) startDialogue(nearAv);
        return;
      }
    }
    ...
    for (let i = 0; i < wModels.length; i++) {
     ...
     if (m.type === 'door_simple') {
  if (doors[m.id]) { closeD(m.id); iMsg('Door closed'); }
  else { openD(m.id); iMsg('Door opened'); }
  return;
}
  ...
  function iMsg(t) { iMsgs.push({ text: t, timer: 3, alpha: 1 }); }
  ...
  socket.on('game-init', (data) => {
    ...
    wAvatars = []; flOn = true; shOn = true;
    collectedNotes = []; currentNoteIndex = -1; noteReading = false; noteText = '';
    dialogueActive = false; dialogueData = null;
    if (dialogueState.typingTimer) { clearInterval(dialogueState.typingTimer); dialogueState.typingTimer = null; }
    dialogueState.currentLine = 0; dialogueState.typingIndex = 0;
    dialogueState.isTyping = false; dialogueState.showingChoices = false;
    dialogueState.completed = new Set(); dialogueState.lastDialogueId = null;
    const dO = document.getElementById('dialogue-overlay'); if (dO) dO.style.display = 'none';
    const nO = document.getElementById('note-overlay'); if (nO) nO.style.display = 'none';
    ...
    if (pd.avatars && Array.isArray(pd.avatars)) {
      pd.avatars.forEach(av => { if (!av) return; wAvatars.push({ id: av.id, x: av.x, y: av.y, w: av.w || 22, h: av.h || 34, direction: av.direction || 1, defaultAnim: av.defaultAnim || 'idle', animSpeed: av.animSpeed || 1, loop: av.loop !== false, interactive: av.interactive || false, usePlayerAvatar: av.usePlayerAvatar || false, bodyColor: av.bodyColor || '#ffffff', headColor: av.headColor || '#ffffff', eyeColor: av.eyeColor || '#000000', keyframes: av.keyframes || [], npcName: av.npcName || (av.dialogue?.npcName) || '', dialogue: av.dialogue || null, properties: av.properties || {} }); });
    }
  ...
  function updateMI() { if (!mobile) return; const ib = document.getElementById('mobile-btn-interact'); if (ib) ib.style.display = (nearI() || dialogueActive) ? 'flex' : 'none'; }
  ...
  window.addEventListener('keydown', (e) => {
    if (noteReading) {
      if (e.key === 'n' || e.key === 'N' || e.key === 'Escape') { hideNote(); e.preventDefault(); return; }
      if (e.key === 'ArrowRight' || e.key === 'ArrowDown') { nextNote(); e.preventDefault(); return; }
      if (e.key === 'ArrowLeft' || e.key === 'ArrowUp') { prevNote(); e.preventDefault(); return; }
      return;
    }
    if ((e.key === 'n' || e.key === 'N') && !chatOn && !escOn && !dialogueActive) { if (collectedNotes.length > 0) { showNote(collectedNotes.length - 1); e.preventDefault(); return; } }
    if (dialogueActive) {
      if (e.key === 'e' || e.key === 'E' || e.key === ' ' || e.key === 'Enter') { advanceDialogue(); e.preventDefault(); return; }
      if (e.key === 'Escape') { endDialogue(); e.preventDefault(); return; }
      if (dialogueState.showingChoices && e.key >= '1' && e.key <= '9') { const d = dialogueData?.dialogue; if (d && d.choices) { const ci = parseInt(e.key) - 1; if (d.choices[ci]) { handleDialogueChoice(d.choices[ci], ci); e.preventDefault(); } } return; }
      return;
    }
  ...
  canvas.addEventListener('mousedown', (e) => { if (!ready || escOn || chatOn || !me || death.on || mobile || noteReading || dialogueActive) return; if (e.button === 0) doAtk(); });
  ...
  function initMobile() {
    ...
    const jz = document.getElementById('mobile-joystick-zone'), je = document.getElementById('mobile-joystick'), ke = document.getElementById('mobile-joystick-knob');
    const JR = 50, DZ = 10; let jtid = null;
    ...
    if (nb) {
      btn(nb, () => { if (noteReading) { if (collectedNotes.length > 1) nextNote(); else hideNote(); } else if (collectedNotes.length > 0) showNote(collectedNotes.length - 1); });
      let noteHoldTimer = null;
      nb.addEventListener('touchstart', () => { noteHoldTimer = setTimeout(() => { if (noteReading) hideNote(); }, 600); }, { passive: true });
      nb.addEventListener('touchend', () => { if (noteHoldTimer) clearTimeout(noteHoldTimer); });
      nb.addEventListener('touchcancel', () => { if (noteHoldTimer) clearTimeout(noteHoldTimer); });
    }
    let noteSwipeX = 0;
    const noteOverlay = document.getElementById('note-overlay');
    if (noteOverlay) {
      noteOverlay.addEventListener('touchstart', (e) => { if (!noteReading) return; noteSwipeX = e.touches[0].clientX; }, { passive: true });
      noteOverlay.addEventListener('touchend', (e) => { if (!noteReading || collectedNotes.length <= 1) return; const dx = e.changedTouches[0].clientX - noteSwipeX; if (Math.abs(dx) > 50) { if (dx < 0) nextNote(); else prevNote(); } }, { passive: true });
    }
  ...
  function updateEsc() {
    ...
    const all = []; if (me) all.push({ ...me, isMe: true }); for (const [, p] of Object.entries(rp)) all.push({ ...p, isMe: false });
    ...
    all.forEach(p => {
      ...
      const mc = document.createElement('canvas'); mc.width = 40; mc.height = 40; av.appendChild(mc);
      const c2 = mc.getContext('2d'); c2.fillStyle = '#111'; c2.fillRect(0, 0, 40, 40);
      TC.draw(c2, 12, 2, 16, 28, 1, 'idle', 0, p.avatar, null, false, Date.now() / 1000, { equipped: p.equipped || {} });
  ...
  document.getElementById('btn-reset').addEventListener('click', () => {
    ...
    collectedNotes = []; currentNoteIndex = -1;
    if (noteReading) hideNote();
    wModels.forEach(m => { if (m.type === 'keycard') delete keycards[m.id]; });
    wCollect.forEach(ci => { if (ci.type === 'note') { ci.done = false; collected.delete(ci.id); } });
  ...
  function updatePlayer(dt) {
    if (!me || !pd || !ready || escOn || chatOn || noteReading || dialogueActive) return;
  ...
  function render() {
    ...
    const ni = nearI();
    for (let i = 0; i < wModels.length; i++) { const m = wModels[i]; if (m.type === 'keycard' && keycards[m.id]) continue; const mw = m.w || 40, mh = m.h || 80; if (m.type.startsWith('door_')) drawDoor(ctx, m, t); else if (m.type === 'lever') drawLever(ctx, m.x, m.y, mw, mh, levers[m.id], t); else if (m.type === 'keycard') drawKeycard(ctx, m.x, m.y, mw, mh, m.properties, t); if (ni && ni.id === m.id) { ctx.fillStyle = 'rgba(255,255,255,0.06)'; ctx.beginPath(); ctx.arc(m.x + mw / 2, m.y + mh / 2, 40 + Math.sin(t * 3) * 5, 0, 6.283); ctx.fill(); if (!mobile) { ctx.fillStyle = 'rgba(255,255,255,0.7)'; ctx.font = 'bold 12px Inter'; ctx.textAlign = 'center'; ctx.textBaseline = 'bottom'; ctx.fillText('[E]', m.x + mw / 2, m.y - 15); } } }
    ...
    for (let i = 0; i < wAvatars.length; i++) { const av = wAvatars[i]; const aw = av.w || 22, ah = av.h || 34; if (!inView(av.x - aw, av.y - ah, aw * 2, ah * 2)) continue; drawGameAvatar(ctx, av, t); if (ni && ni.id === av.id && av.interactive && !dialogueActive) { ctx.fillStyle = 'rgba(255,255,255,0.06)'; ctx.beginPath(); ctx.arc(av.x, av.y, 40 + Math.sin(t * 3) * 5, 0, 6.283); ctx.fill(); if (!mobile) { ctx.fillStyle = 'rgba(255,255,255,0.7)'; ctx.font = 'bold 12px Inter'; ctx.textAlign = 'center'; ctx.textBaseline = 'bottom'; ctx.fillText('[E]', av.x, av.y - ah / 2 - 15); } } }
    ...
    for (let i = 0; i < H.msgs.length; i++) { const m = H.msgs[i]; ctx.save(); ctx.globalAlpha = m.alpha; ctx.font = '900 28px Inter'; ctx.textAlign = 'center'; ctx.fillStyle = '#CC2222'; ctx.shadowColor = 'rgba(255,0,0,0.5)'; ctx.shadowBlur = 10; ctx.fillText(m.text, canvas.width / 2, canvas.height / 2 - 60 + i * 40); ctx.shadowBlur = 0; ctx.restore(); }
    for (let i = 0; i < iMsgs.length; i++) { const m = iMsgs[i]; ctx.save(); ctx.globalAlpha = m.alpha; ctx.font = '14px Inter'; ctx.textAlign = 'center'; const y = canvas.height - 100 - i * 30, w = ctx.measureText(m.text).width + 20; ctx.fillStyle = 'rgba(0,0,0,0.7)'; ctx.fillRect(canvas.width / 2 - w / 2, y - 12, w, 24); ctx.fillStyle = '#fff'; ctx.textBaseline = 'middle'; ctx.fillText(m.text, canvas.width / 2, y); ctx.restore(); }
  ...
  function renderDark(t) {
    ...
    for (let i = 0; i < wAvatars.length; i++) { const av = wAvatars[i]; const ax = av.x - cam.x, ay = av.y - cam.y; if (ax < -50 || ax > W + 50 || ay < -50 || ay > H2 + 50) continue; const gg = d.createRadialGradient(ax, ay, 0, ax, ay, 25); gg.addColorStop(0, 'rgba(0,0,0,0.3)'); gg.addColorStop(1, 'rgba(0,0,0,0)'); d.fillStyle = gg; d.beginPath(); d.arc(ax, ay, 25, 0, 6.283); d.fill(); }
  ...
  function drawInv(t) {
    ...
    for (let i = 0; i < inv.length; i++) { const x = sx + i * (ss + gap), a = i === me.activeSlot; ctx.fillStyle = a ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.6)'; ctx.fillRect(x, sy, ss, ss); ctx.strokeStyle = a ? '#fff' : '#222'; ctx.lineWidth = a ? 2 : 1; ctx.strokeRect(x, sy, ss, ss); if (inv[i]) { const v = gv(inv[i].id); v.dH(ctx, x, sy, ss); ctx.font = '600 8px Inter'; ctx.textAlign = 'center'; let l = v.name; if (a && inv[i].id === 'flashlight') { l += flOn ? ' ON' : ' OFF'; ctx.fillStyle = flOn ? '#FFE066' : '#555'; } else if (a && inv[i].id === 'shield') { l += shOn ? ' UP' : ' DOWN'; ctx.fillStyle = shOn ? '#66AAEE' : '#555'; } else ctx.fillStyle = '#666'; ctx.fillText(l, x + ss / 2, sy + ss - 4); } ctx.font = '600 9px Inter'; ctx.textAlign = 'left'; ctx.fillStyle = a ? '#fff' : '#333'; ctx.fillText(String(i + 1), x + 4, sy + 12); }
    const ai = inv[me.activeSlot]; if (ai && gv(ai.id).tog) { ctx.font = '500 10px Inter'; ctx.textAlign = 'center'; ctx.fillStyle = 'rgba(255,255,255,0.3)'; ctx.fillText('[F] toggle', canvas.width / 2, sy - 8); }
    if (nearI() && !mobile && !dialogueActive) { ctx.font = '500 10px Inter'; ctx.textAlign = 'center'; ctx.fillStyle = 'rgba(255,255,255,0.4)'; ctx.fillText('[E] interact', canvas.width / 2, sy - 22); }
    if (collectedNotes.length > 0 && !mobile) { ctx.font = '500 10px Inter'; ctx.textAlign = 'center'; ctx.fillStyle = 'rgba(255,255,255,0.3)'; ctx.fillText(`[N] notes (${collectedNotes.length})`, canvas.width / 2, sy - 36); }
    if (me.hp !== undefined) { const bw = 200, bh2 = 8, bx2 = canvas.width / 2 - bw / 2, by2 = sy - (collectedNotes.length > 0 ? 50 : 36); ctx.fillStyle = '#111'; ctx.fillRect(bx2, by2, bw, bh2); const p3 = Math.max(0, me.hp / (me.maxHp || 100)); ctx.fillStyle = p3 > 0.5 ? '#4ade80' : p3 > 0.25 ? '#fbbf24' : '#ef4444'; ctx.fillRect(bx2, by2, bw * p3, bh2); ctx.font = '600 10px Inter'; ctx.textAlign = 'center'; ctx.fillStyle = '#666'; ctx.fillText(`${Math.max(0, Math.round(me.hp))} HP`, canvas.width / 2, by2 - 4); }
  }
